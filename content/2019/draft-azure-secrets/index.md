...
While the concepts are universally applicable, my code samples focus a .NET application running in Azure App Service and configured with Pulumi.
...
Say, you are testing integration with a payment service:
...
Both snippets are fine for a Hello-World application with a lifespan of two hours. However, I would strongly recommend against doing so in any code that can potentially be checked into a source control system, even for a 10-minute experiment. Don't copy-paste secrets into the files which are part of a git repository. One accidental `git commit & git push`&mdash;and the secrets are compromised.

Avoid the "Secrets as Code" practice: there are bots scanning your GitHub for those.

## 2. Configuration Files

Traditionally, a more respected way would be to put secrets in a configuration file. For classic .NET applications, this would be an `app.config` or a `web.config` file. The idea is that the values on a developer machine are different from the values in the production environment. Non-sensitive development settings are kept on disk and maybe in source control, while the real secrets are only injected by a deployment script or a CI/CD system, so they are not exposed publicly.
...
which is then mapped to a POCO in code:
...
On top of that, it doesn't feel right to mix configuration (as in, how many threads to run, or how big the message batches should be) with secret connection strings and API keys in the same file. These are separate kinds of configuration, and they warrant different workflows.
...
A great feature of the App Service is the ability to set environment variables via Application Settings. Here is a snippet of a Pulumi program that passes both secret values to `appSettings`:
...
The payment service key is provided by a third-party, so its encrypted value is stored in Pulumi configuration. Read [Managing Secrets with Pulumi](https://www.pulumi.com/blog/managing-secrets-with-pulumi/) to learn about all security options available for secrets in Pulumi config.

## 4. Azure Key Vault

In the previous example, both secrets end up in Application Settings. Every person with sufficient permissions may go the App Service and see them in clear text. While this can be restricted, it's usually a good idea to grant full read access to the application developers and operators of the App Service to make sure they have the full picture while troubleshooting any issues.

That is why there is a dedicated Azure service to store the secrets: Azure Key Vault. A Key Vault can be created and populated with all the secrets from the same Pulumi program:
...
Now, one or many Service Principals (SP) should be configured to access the Key Vault. One SP deploys the infrastructure by running the Pulumi program: it has to be granted access to the Key Vault to manage secrets in it. That's why the snippet above assigns an access policy to `currentPrincipal`.
...
This solution is not entirely satisfying though: we've traded storing the secrets for storing SP credentials. Is that a big enough win?  Luckily, it's easy to get rid of those credentials with Managed Service Identity.
...
Now, the configuration block in the .NET Core app needs no secrets to retrieve:
...
That's quite a bit of boilerplate, but there is a way to get rid of it.

## 6. Accessing Key Vault from Application Settings

App Service has a neat feature of integrating its Application Settings with Key Vault. It allows us to combine the approach from #3 and #5 and get the best of both:
...
Now, the API key is loaded into the App Service environment variable without its value being publicly exposed anywhere!
...
- It links an output of one resource to the input of another one, avoiding the need to store the values;
